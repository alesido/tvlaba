...
class VodPlaybackFooterViewModel @Inject constructor (
    ...
    private val liveData: MutableLiveData<Resource<VodPlaybackFooterLiveData>> = MutableLiveData()
    private val snapshot = VodPlaybackFooterLiveData()
    ...
    fun getLiveData(): LiveData<Resource<VodPlaybackFooterLiveData>> = liveData
    ...
    fun onSeriesItemSelected(seriesIndex: Int) {

    }

    fun onSeriesItemAction(seriesIndex: Int) {
    ...
    inner class VodItemSubscriber : DisposableSingleObserver<VodListingItem>() {
        override fun onSuccess(t: VodListingItem) {
            ...
            snapshot.updateScope = VodPlaybackFooterUpdateScope.PLAYBACK_DETAILS
    ...
    private inner class ListingPageSubscriber: DisposableSingleObserver<VodListingPage>() {
        override fun onSuccess(page: VodListingPage) {
            ...
            snapshot.cursor?.unit?.let {
                it.window?.add(page)
                snapshot.updateScope = VodPlaybackFooterUpdateScope.LISTING
                liveData.postValue(Resource.success(snapshot))
