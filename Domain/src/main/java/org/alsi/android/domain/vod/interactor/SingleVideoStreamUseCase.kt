...
class SingleVideoStreamUseCase  @Inject constructor(
    private val presentationManager: PresentationManager,
    executionThread: PostExecutionThread
): SingleObservableUseCase<VideoStream, SingleVideoStreamUseCase.Params?>(executionThread) {
    ...
    override fun buildUseCaseObservable(params: Params?): Single<VideoStream> {
        ...
        return if (repo is VodRepository) {
            repo.getSingleVideoStream(params.vodItemId)
        }
        else {
